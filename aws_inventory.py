...
#import aws_inventory.json_to_csv
...
def parse_args(args=None):
    ...
    parser.add_argument('--csv-file-template',
                        help='Template filename to convert the GUI JSON data to CSV format',
                        default=aws_inventory.config.CSV_DATA_FILENAME_TEMPLATE.template)
    ...
    if type(parsed.gui_file_template) is str:
        ...
        parsed.csv_file_template = string.Template(parsed.csv_file_template)
...
def main(args):
    ...
    if args.list_operations:
        ...
        for profile in profiles:
            ...
            csv_data_file       = data_file_name(args.csv_file_template, profile)        if 'gui' in args.output else None
            ...
            aws_inventory.invoker.ApiInvoker(args, service_descriptors, ops_count, gui_data_file, response_data_file, exception_data_file, csv_data_file).probe_services()
