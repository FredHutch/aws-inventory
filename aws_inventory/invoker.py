...
class ApiInvoker(object):
    ...
    def __init__(self, script_args, svc_descriptors, ops_count, gui_data_file, response_data_file, exception_data_file, csv_data_file):
    ...
    def write_results(self):
        ...
        :param file csv_data_fp: file for CSV data
        """
        if not self.script_args.dry_run:
            ...
            if self.gui_data_file is not None:
                with open(self.gui_data_file, 'w') as out_fp, open (self.csv_data_file, 'w') as out_cp:
                    self.store.generate_data_file(out_fp, out_cp)
